...
class ExactVRPAgent(Agent):
    ...
    def compute_VRP(self, delivery_to_do, vehicles_dict, gap=None, time_limit=None, verbose=False, debug_model=False):
        ...
        nodes = {}
        to_assign = []     
        to_assign_array = np.zeros((len(delivery_to_do.items()),5), float)
        ...
        for key, ele in delivery_to_do.items():
            ...
            to_assign_array[c_][1] = ele['time_window_max']
            to_assign_array[c_][2] = ele['id']
            to_assign_array[c_][3] = 0
            to_assign_array[c_][4] = ele['vol']
        ...
        for v in vehicles_dict:
            vehicles[v_index] = {'capacity': float(v['capacity']),
            'cost': v['cost'],
            'tw_min': 99999,
            'tw_max': -1,
            'route': [],
        ...
        for v in vehicles:
            if vehicles[v]['capacity'] > max_capacity:
                max_capacity = vehicles[v]['capacity']
                chosen_vehicle = vehicles[v]['id']
        
        vehicles[chosen_vehicle]['route'].append(int(to_assign_array[0][2]))
        to_assign_array[0][3] = 1
        vehicles[chosen_vehicle]['tw_min'] = to_assign_array[0][0]
        vehicles[chosen_vehicle]['tw_max'] = to_assign_array[0][1]
        vehicles[chosen_vehicle]['capacity'] -= to_assign_array[0][4]
        

        assigned_old = 0

        while  assigned < len(delivery_to_do.items()):
            
                for v in vehicles:
                    if len(vehicles[v]['route']) > 0 and vehicles[v]['capacity'] >= 1 and assigned != assigned_old:
                        break
                    elif len(vehicles[v]['route']) == 0 and vehicles[v]['capacity'] >= 1:
                        break
                ...
                ass = 0
                flag = 0
                for add in range(0,26):
                    for i in range(len(to_assign_array)):
                        if (flag == 0 and to_assign_array[i][3] == 0 and len(vehicles[v]['route']) > 0 and ((to_assign_array[i][0] ==  (vehicles[v]['tw_max'] + add )) or (to_assign_array[i][0] ==  (vehicles[v]['tw_max'] + add+1)) or (to_assign_array[i][0] ==  (vehicles[v]['tw_max'] + add+2))) and vehicles[v]['capacity'] >= to_assign_array[i][4]):
                            cost = (self.distance_matrix[0][int(to_assign_array[i][2])])- (self.distance_matrix[len(vehicles[v]['route'])][int(to_assign_array[i][2])])
                            if cost > cost_max:
                                cost_max = cost
                                next_point = i
                                ass = 1
                        ...
                        elif (flag == 0 and to_assign_array[i][3] == 0 and vehicles[v]['capacity'] >= to_assign_array[i][4] and len(vehicles[v]['route']) == 0):   
                            vehicles[v]['route'].append(int(to_assign_array[i][2]))
                            vehicles[v]['tw_min'] = to_assign_array[i][0]
                            vehicles[v]['tw_max'] = to_assign_array[i][0]
                            vehicles[v]['capacity'] = vehicles[v]['capacity'] - to_assign_array[i][4]
                            to_assign_array[i][3] = 1
                            assigned += 1
                            #print(assigned, to_assign_array[i][2], v)
                            flag = 1
                    ...
                    """"
                    elif (to_assign_array[i][3] == 0 and vehicles[v]['capacity'] < to_assign_array[i][4]):    #if no more room, fid an empty vehicle
                        max_capacity = 0
                        for v_ind in vehicles:
                            if vehicles[v_ind]['capacity'] > max_capacity and len(vehicles[v_ind]['route']) == 0:
                                max_capacity = vehicles[v_ind]['capacity']
                                v_ = vehicles[v_ind]['id']

                        vehicles[v_]['route'].append(int(to_assign_array[i][2]))
                        vehicles[v_]['tw_min'] = min(vehicles[v_]['tw_min'], to_assign_array[i][0])
                        vehicles[v_]['tw_max'] = max(vehicles[v_]['tw_min'], to_assign_array[i][0])
                        vehicles[v_]['capacity'] = vehicles[v_]['capacity'] - to_assign_array[i][4]
                        to_assign_array[i][3] = 1
                        assigned += 1
                        print(assigned, to_assign_array[i][2], v_)
                    ...
                    if ass == 1:
                        vehicles[v]['route'].append(int(to_assign_array[next_point][2]))
                        vehicles[v]['tw_min'] = min(vehicles[v]['tw_min'], to_assign_array[next_point][0])
                        vehicles[v]['tw_max'] = max(vehicles[v]['tw_max'], to_assign_array[next_point][0])
                        vehicles[v]['capacity'] = vehicles[v]['capacity'] - to_assign_array[next_point][4]
                        to_assign_array[next_point][3] = 1
                        assigned += 1
                        #print(assigned, to_assign_array[next_point][2], v)
                        break
            

        
        for v in vehicles:
            if len(vehicles[v]['route']) > 0:
                vehicles[v]['route'].insert(0,0)

        solution_final = []
        for i in range(len(vehicles)):
            solution_final.append(vehicles[i]['route'])
